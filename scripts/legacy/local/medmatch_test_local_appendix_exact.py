#!/usr/bin/env python3
"""
MedMatch LLM Evaluation Script - dataset-aligned appendix-style local baseline.
Runs the appendix-style one-shot prompts against the local Ollama model while
preserving the project scoring logic and dataset loader.
"""
...
def build_instruction(sheet_name, base_instruction, prompt, expected_keys):
    """Return the appendix prompt text without later prompt augmentations."""
...
def main():
    ...
    print("Prompt set: dataset-aligned appendix-style one-shot")
    ...
    for sheet_name, entries in dataset.items():
        ...
        out_path = os.path.join(
            results_dir, f"{safe_name}_dataset_aligned_appendix_style_{timestamp}.json"
        )
        ...
        csv_path = os.path.join(
            results_dir, f"{safe_name}_dataset_aligned_appendix_style_{timestamp}.csv"
        )
